package esvmcompiler.parcer;

import esvmcompiler.asmgen.Asmgen;
import esvmcompiler.files.Files;
import esvmcompiler.inter.*;
import esvmcompiler.lexer.*;
import esvmcompiler.main.Infos;
import esvmcompiler.symbols.Array;
...
public class Parcer {
    ...
    private ArrayList<Env> flatenv = new ArrayList<Env>();
    private int used = 0;

    public Parcer(Lexer l) throws IOException {
        lex = l;
        move();
    }
    ...
    public void program() throws IOException {
        match(Tag.CLASS);
        Infos.classname = look.toString();
        Files.setOutFile(new File("/home/serbis/Projects/Outline/EsvmCompiler/" + Infos.classname + ".tac"));
        Token tok = look;
        match(Tag.ID);
        top = new Env(top);
        Id id = new Id((Word) tok, Type.Int, tok.tag);
        top.put(tok, id);
        Stmt s = block();
        ...
        s.emitlabel(after);
        Files.closeFile();
        //Генерируем asm представление
        Asmgen asmgen = new Asmgen(new File("/home/serbis/Projects/Outline/EsvmCompiler/" + Infos.classname + ".tac"), flatenv.get(0));
        asmgen.generate();
    }
    ...
    private Stmt stmt() throws IOException {
        ...
        switch (look.tag) {
            ...
            case Tag.WHILE:
                ...
                savedStmt = Stmt.Encosing;
                Stmt.Encosing = whilenode;
                match(Tag.WHILE);
                match('(');
                x = bool();
                match(')');
                s1 = stmt();
                whilenode.init(x, s1);
                Stmt.Encosing = savedStmt;
                return whilenode;

            case Tag.DO:
                Do donode = new Do();
                savedStmt = Stmt.Encosing;
                Stmt.Encosing = donode;
                match(Tag.DO);
                s1 = stmt();
                match(Tag.WHILE);
                match('(');
    ...
    private Assess offset(Id a) throws IOException {
        Expr i;
        Expr w;
        Expr t1;
        Expr t2;
